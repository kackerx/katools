...
func main() {
	...
	dsn := "kingvstr_dy:M3KHa7fjRkRjkkmJ@tcp(27.102.66.66:3306)/kingvstr_dy?charset=utf8"
	...
	for _, item := range videos {
		if !strings.Contains(item.Pic, "mgtv") && !strings.Contains(item.Pic, "alicdn") && !strings.Contains(item.Pic, "hulupa") && !strings.Contains(item.Pic, "yking") {
	...
	for _, v := range upDatas {
		go func(video Video) {
			defer wg.Done()
			if imgUrl, err = katools.UploadImgTx(video.Pic); err != nil {
				fmt.Println(err)
				return
			}
			video.Pic = imgUrl
